...
3. **Run the benchmark**
   ...
   - *"Which preselected model would you like to benchmark?"* → LLM, VLM, MoE, or MiniCPM
   - *"Which precision would you like to serve?"* → bf16 or int4 (supported
     by both engines: OVMS int4 uses native OpenVINO IR weight compression;
     vLLM int4 quantizes the bf16 checkpoint on the fly with torchao)
...
- Edit [`config/models.yaml`](../../config/models.yaml) to change the
  ...
  arguments (e.g., `--max-model-len`). Both engines define `bf16` and `int4`
  per model: OVMS under `models.<type>.ovms.precisions.*` (`--weight-format`
  export args: `fp16` for bf16, `int4` for int4), and vLLM under
  `models.<type>.vllm.precisions.*` (`bf16` has an `hf_repo`; `int4` uses
  `quantization: torchao` and is produced on the fly from the bf16 checkpoint
  — no separate pre-quantized repo needed).
